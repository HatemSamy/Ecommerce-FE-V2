...
//         const { name, amount, discount, CategoryId, BrandId, price } = req.body
...
export const createproduct = asynchandiler(async (req, res, next) => {
    ...
    for (const file of req.files) {
        const { secure_url, public_id } = await cloudinary.uploader.upload(file.path, {folder:`onlineEcommerce/product/${name.trim()}`})
        images.push(secure_url)
        imagepublicIds.push(public_id)
    }
    req.body.images = images
...
export const update = asynchandiler(
    ...
    async (req, res, next) => {
        ...
        const { name, amount, price, discount, CategoryId, BrandId } = req.body
        if (name) {
            req.body.slug = slugify(name)
        }
        ...
        if (CategoryId) {
            const category = await findById({ model: subCategorymodel, filter: {CategoryId } })
